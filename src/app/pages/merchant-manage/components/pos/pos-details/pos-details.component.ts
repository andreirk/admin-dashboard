...
export class PosDetailsComponent implements OnInit {
  ...
  ngAfterViewInit() {
    ...
    vm.form.control.valueChanges.debounceTime(500)
      .subscribe(values => {
        ...
        vm.model.mapMarkersViewModel = PosViewModelService.updatePosMarkerViewModel(vm.model.showResolved, vm.model.pos.address.geoPoint, vm.model.resolvedPoint);
      });
  }
  ...
  onMarkerClicked(event) {
    ...
    if (vm.model.showResolved) {
      vm.onPointChosen({chosenPoint: {lat: vm.model.mapMarkersViewModel[event].lat, lon: vm.model.mapMarkersViewModel[event].lon}});
    }
